...
?>
<div class="border border-primary rounded-top">
<div class="bg-primary border border-gray"><h3 class="ml-2">前端模板设置</h3></div>
<div class="bg-white px-2 py-3">
  <p class="text-muted">主题目录位于 <code>templates/</code>。缺页会自动回退到 <code>default</code>。</p>

  <div class="form-group">
    <label>用户端主题（控制面板）</label>
    ...
    <small class="form-text text-muted">当前：<?=htmlspecialchars($curUserTheme)?></small>
  </div>

  <div class="form-group">
    <label>管理端主题（后台）</label>
    ...
    <small class="form-text text-muted">当前：<?=htmlspecialchars($curAdminTheme)?></small>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="table-responsive">
      <table class="table table-bordered table-sm">
        <thead><tr><th>目录名</th><th>名称</th><th>版本</th><th>用户端</th><th>管理端</th><th>说明</th></tr></thead>
        <tbody>
        <?php
        ...
        <?php else: foreach ($all as $t): ?>
          <tr>
            ...
            <td><?=!empty($t['has_user']) ? '<span class="text-success">是</span>' : '<span class="text-muted">否</span>'?></td>
            <td><?=!empty($t['has_admin']) ? '<span class="text-success">是</span>' : '<span class="text-muted">否</span>'?></td>
  ...
  <button class="mb-2 btn btn-primary form-control" type="button" onclick="settheme()"><label><i class="mdi mdi-checkbox-marked-circle-outline"></i></label>保存主题设置</button>
  <p class="text-muted small mb-2">保存后用户端立即生效；管理端建议刷新后台页面。</p>
</div>
</div>
<?php
}
